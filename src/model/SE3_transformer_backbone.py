...
from model.utils.utils_EGNN import GaussianSmearing, MLP, batch_hybrid_edge_connection, outer_product
...
class SE3Transformer(nn.Module):
    def __init__(self, num_blocks, num_layers, input_dim, hidden_dim, n_heads=1, k=32,
                 num_r_gaussian=50, edge_feat_dim=0, num_node_types=8, act_fn='relu', norm=True,
                 cutoff_mode='radius', ew_net_type='r',
                 num_init_x2h=1, num_init_h2x=0, num_x2h=1, num_h2x=1, r_max=10., x2h_out_fc=True, sync_twoup=False, time_cond=True):
        super().__init__()
        # Build the network
    ...
    def forward(self, h, x, t, Gt_mask, batch, return_all=False, fix_x=False):
        '''
            h: protein node features + ligand node features
            x: protein node positions + ligand node positions
        ...
        for b_idx in range(self.num_blocks):
            edge_index = self._connect_edge(x, Gt_mask, batch)
            src, dst = edge_index
